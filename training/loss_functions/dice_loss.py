...
class DC_and_CE_KL_Loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,weight_kl=0.2, weight_mse=0.2,
                 log_dice=False, ignore_label=None):
        """
        CAREFUL. Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        :param soft_dice_kwargs:
        ...
        :param weight_dice:
        """
        super(DC_and_CE_KL_Loss, self).__init__()
        if ignore_label is not None:
            assert not square_dice, 'not implemented'
            ce_kwargs['reduction'] = 'none'
        self.log_dice = log_dice
    ...
    def forward(self, net_output, target):
        ...
        # print('vae_pred size',vae_pred.size())
        # print('vae_distr size',vae_distr.size())
